...
fish_types = {
        ...
        # fishtype: (swerving, un_decisiveness, f_speed, f_id, cautiousness, fishname, nesting, baitlist)
        'maternal_salmon': (2, 2, 0.4, 1, 0.4, 'maternal_salmon', (50, 0, 1, 1),# nesting: range, speed, min max
                            {  # baitlist
                                'worms': 0.8,
                                'plastic_bait': 0.7,
                                'insect_bait': 0.8,
...
class Fish(pygame.sprite.Sprite):
    ...
    fish_lists = {} # the only comprehensive collection of active fish.
    for fish in fish_types.keys():
        fish_lists[fish] = pygame.sprite.Group()

    fish_caught = False # checks if a fish is circling the hook
    fish_took = False # keeps track of if a fish is on the hook

    fish_square_width = 16

    @classmethod
    def stop_fishing(cls):
        """resets fishing related variables to false. This should ONLY be called in player.stop_fishing!!!! """
    ...
    def rescale(cls):
        ...
        cls.fish_square_width = scale(cls.fish_square_width)
        for key, value in fish_frames.items():
            fish_frames[key] = pygame.transform.scale(value, (cls.fish_square_width, cls.fish_square_width))
        for species_list in cls.fish_lists.values():
            for fish in species_list:
                # arbitrary starting image.
                fish.image = pygame.transform.scale(load_asset('fishleft1.png','fish scheiße'), (cls.fish_square_width, cls.fish_square_width))
                fish.rect = fish.image.get_rect()

    @classmethod
    def create_fish(cls, cords, spritelist, *args):
        """creates a new fish instance in the fish_list sprite group"""
        new = Fish(cords, *args)
        cls.fish_lists[args[6]].add(new) #args 6 is the item argument
        spritelist.add(new)
        return new
    ...
    def complex_fish_movement(self, player, timer, inventory, game_state, grid_ahead, yp, xp):
        bait = inventory.inventory.bait_slot # renaming
        if timer % 10 == 0:  # handles expensive operations such as swerving and baiting 6x a second.
            self.expensive_fish_movement(player, bait, timer, inventory, game_state)
        if self != Fish.fish_caught:
            self.fish_move(grid_ahead, xp, yp)

    def __init__(self, cords, inventory, swerving, un_decisiveness, f_speed, f_id, cautiousness, item, origin_bound, bait_dict):
        super().__init__()
    ...
    def check_hook_collision(self, player_hook_cords):
        if player_hook_cords and not Fish.fish_caught:
            ...
            self.rect.topleft = self.cords  # updates the fishes hit-boxes. This only happens when a hook is out to save resources
    ...
    def fish_move(self, grid_ahead, xp, yp):
        ...
        LocoLock.move_swarms(grid_ahead, xp, yp)
        x = self.cords[0]
        y = self.cords[1]
        new_vector = self.vector
        ranlist = (-1, 1)
